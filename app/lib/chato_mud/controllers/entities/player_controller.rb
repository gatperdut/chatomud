require "chato_mud/mixins/players/login"
require "chato_mud/mixins/players/main_menu"
require "chato_mud/controllers/entities/entity_controller"
require "chato_mud/controllers/characters/pc_controller"
  ...
  module Controllers
    module Entities
      class PlayerController < EntityController
        ...
        def is_logging_in?
          [:providing_nickname, :providing_password].include?(@state)
        end
        ...
        def wait_for_reconnection
          ...
          Rails.logger.warn("#{@player.nickname} disconnected. Thread sleeping ...")
          sleep(10.seconds)
          if @waiting_for_reconnection
            Rails.logger.warn("#{@player.nickname}'s link dead too long. Closing connection.")
            bye(true)
          end
          tx("\nWelcome back, #{@player.nickname}.\n")
          @character_controller.emit_reconnection
          Rails.logger.warn("#{@player.nickname} has reconnected.")
          Actions::LookAround.new(@server, @character_controller, nil).exec
        end
