class Room < ApplicationRecord
  ...
  def anchor(destroying)
    ...
    update(er: Room.find(er_id)) unless er_id.nil? || destroying
    update(sr: Room.find(sr_id)) unless sr_id.nil? || destroying
    update(wr: Room.find(wr_id)) unless wr_id.nil? || destroying
    update(ur: Room.find(ur_id)) unless ur_id.nil? || destroying
    update(dr: Room.find(dr_id)) unless dr_id.nil? || destroying

    r = Room.find_by_nr_id(id)
    r.update(nr_id: nil) if r.present? && (sr_id.nil? || destroying)

    r = Room.find_by_er_id(id)
    r.update(er_id: nil) if r.present? && (wr_id.nil? || destroying)

    r = Room.find_by_sr_id(id)
    r.update(sr_id: nil) if r.present? && (nr_id.nil? || destroying)

    r = Room.find_by_wr_id(id)
    r.update(wr_id: nil) if r.present? && (er_id.nil? || destroying)

    r = Room.find_by_ur_id(id)
    r.update(ur_id: nil) if r.present? && (dr_id.nil? || destroying)

    r = Room.find_by_dr_id(id)
    r.update(dr_id: nil) if r.present? && (ur_id.nil? || destroying)
  end
