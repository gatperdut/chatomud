class Door < ApplicationRecord
  ...
  def anchor(destroying)
    ...
    r.update(sd_id: destroying ? nil : id) if r.present?
    r = Room.find_by_id(sr_id)
    r.update(nd_id: destroying ? nil : id) if r.present?

    r = Room.find_by_id(er_id)
    r.update(wd_id: destroying ? nil : id) if r.present?
    r = Room.find_by_id(wr_id)
    r.update(ed_id: destroying ? nil : id) if r.present?

    r = Room.find_by_id(ur_id)
    r.update(dd_id: destroying ? nil : id) if r.present?
    r = Room.find_by_id(dr_id)
    r.update(ud_id: destroying ? nil : id) if r.present?
  end
